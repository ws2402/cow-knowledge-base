...
import sys
  ...
  class VectorStore:
   def __init__(self,persist_directory="./chroma_db"):
    ...
    try:self.collection=self.client.get_collection("knowledge_base")
    except:self.collection=self.client.create_collection(name="knowledge_base")
   def add_document(self,content,metadata):
    try:
     doc_id=hashlib.md5(content.encode('utf-8')).hexdigest()
     existing=self.collection.get(ids=[doc_id])
     if existing['ids']:return doc_id
     self.collection.add(documents=[content],metadatas=[metadata],ids=[doc_id])
     return doc_id
    except Exception as e:logger.error(f"添加失败:{e}");return None
    ...
    try:
     ...
     if results['documents'] and results['documents'][0]:
      for i in range(len(results['documents'][0])):
       formatted_results.append({'content':results['documents'][0][i],'metadata':results['metadatas'][0][i] if results['metadatas'][0] else {},'distance':results['distances'][0][i] if results['distances'][0] else 
  0})
  ...
  class ArticleParser:
   ...
   def is_wechat_article(self,url):return 'mp.weixin.qq.com' in url
   def parse_article(self,url):
    if not self.is_wechat_article(url):return None
    try:
     ...
     if len(text_content.strip())<100:return None
     return {'title':title,'content':text_content,'url':url,'metadata':{'type':'wechat_article','url':url,'source':'微信公众号'}}
    except Exception as e:logger.error(f"解析失败:{e}");return None
   ...
   def chunk_content(self,content,chunk_size=1000,overlap=100):
    ...
    while start<len(content):
     ...
     if end<len(content):
      ...
      if last_period>start:end=last_period+1
  ...
  class PodcastParser:
   ...
   def is_xiaoyuzhou_link(self,url):return 'xiaoyuzhoufm.com' in url
   def parse_podcast(self,url):
    if not self.is_xiaoyuzhou_link(url):return None
    try:
     podcast_info=self._get_podcast_info(url)
     if not podcast_info:return None
     audio_file=self._download_audio(podcast_info['audio_url'])
     if not audio_file:return None
     try:
      transcript=self._transcribe_audio(audio_file)
      if not transcript:return None
      return {'title':podcast_info['title'],'content':transcript,'url':url,'metadata':{'type':'xiaoyuzhou_podcast','url':url,'source':'小宇宙播客'}}
     finally:
      if os.path.exists(audio_file):os.remove(audio_file)
    except Exception as e:logger.error(f"解析播客失败:{e}");return None
   def _get_podcast_info(self,url):
    ...
    except Exception as e:logger.error(f"获取播客信息失败:{e}");return None
   def _extract_audio_url(self,soup,page_url):
    audio_tag=soup.find('audio')
    if audio_tag and audio_tag.get('src'):return audio_tag.get('src')
   ...
   def _download_audio(self,audio_url):
    ...
    except Exception as e:logger.error(f"下载音频失败:{e}");return None
   def _transcribe_audio(self,audio_file):
    try:
     with open(audio_file,'rb') as f:
      transcript=self.openai_client.audio.transcriptions.create(model="whisper-1",file=f,language="zh")
  ...
  logger=logging.getLogger(__name__)
  @plugins.register(name="KnowledgeBase",desire_priority=10,hidden=False,desc="智能知识库插件",version="1.0")
  class KnowledgeBasePlugin(Plugin):
   def __init__(self):
    super().__init__()
    try:
     config=super().load_config() or {"enabled":True,"openai_api_key":"","chroma_persist_directory":"./chroma_db","max_results":5,"similarity_threshold":0.7}
    ...
    except Exception as e:logger.error(f"插件初始化失败:{e}")
   def on_handle_context(self,e_context):
    try:
     context=e_context['context']
     if context.type not in [ContextType.TEXT]:return
     content=context.content.strip()
     urls=re.findall(r'https?://[^\\s\\u4e00-\\u9fff]+',content)
     if urls:self._process_urls(urls,e_context)
     else:self._search_knowledge_base(content,e_context)
     ...
     try:
      if self.article_parser.is_wechat_article(url):
       ...
       if result:
        ...
        for chunk in chunks:self.vector_store.add_document(chunk,result['metadata'])
        processed_count+=1
       ...
       if result:
        ...
        processed_count+=1
     except Exception as e:logger.error(f"处理URL失败:{e}")
